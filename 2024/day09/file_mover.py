...
def flatten_by_chunk2():
    ...
    while i < len(output):
        if output[i] == ".":
            ...
            while output[i] == ".":
                space_avail += 1
                i += 1
            indexes_avail.append(i - space_avail)
            spaces_avail.append(space_avail)
        i += 1
    ...
    for i in range(len(data) // 2):
        current_id = output[p2]
        space_needed = count_space_needed(output, current_id, p2)
        print(current_id, space_needed)
        p2 = swap_chunk(output, spaces_avail, indexes_avail, space_needed, p2)
        print(p2, output[p2])
        if current_id == output[p2]:  # if the placement failed, skip it
            p2 -= space_needed
            while output[p2] == ".":
...
def swap_chunk2(output, spaces_avail, indexes_avail, space_needed, p2):
    for i in range(len(spaces_avail)):
        if spaces_avail[i] >= space_needed:
            for j in range(space_needed):
                output[indexes_avail[i] + j], output[p2] = (
                    output[p2],
                    output[indexes_avail[i] + j],
                )
                p2 -= 1
            if spaces_avail[i] == space_needed:
                indexes_avail.pop(0)
                ...
                indexes_avail[i] = indexes_avail[i] + space_needed
                spaces_avail[i] = spaces_avail[i] - space_needed
            break
    return p2
