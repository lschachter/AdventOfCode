...
rules: Dict[str, List[str]] = {rule[4]: [rule[0], rule[2], rule[1]] for rule in rules}


def build_number(gates: dict) -> int:
    for rule, vals in rules.items():
        gates[rule] = get_gate(*vals)

    gates: List[str] = list(gates.items())
...
def get_gate(gate_1: str, gate_2: str, operator: str) -> str:
    right: str = gates.get(gate_1) or get_gate(*rules[gate_1])
    left: str = gates.get(gate_2) or get_gate(*rules[gate_2])
